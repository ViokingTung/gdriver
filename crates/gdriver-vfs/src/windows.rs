...
//
// When the `winfsp-vfs` feature is enabled, this module provides a full
// WinFSP-backed virtual filesystem. Otherwise it exposes stubs that
// return errors so the crate still compiles on Windows CI without WinFSP.

// ─── Stub (winfsp-vfs feature disabled) ──────────────────────────────────

#[cfg(not(feature = "winfsp-vfs"))]
mod stub {
    ...
    impl VfsBackend for WindowsVfsBackend {
        async fn mount(&self, _ctx: VfsContext) -> anyhow::Result<VfsHandle> {
            anyhow::bail!("WinFSP support not enabled. Enable the 'winfsp-vfs' feature.")
        }

        async fn unmount(_handle: VfsHandle) -> anyhow::Result<()> {
            Ok(())
        }
    }

    pub async fn mount_winfsp(_ctx: VfsContext) -> anyhow::Result<VfsHandle> {
        anyhow::bail!("WinFSP support not enabled. Enable the 'winfsp-vfs' feature.")
    }

    pub fn unmount_winfsp(_mount_point: &std::path::Path) -> anyhow::Result<()> {
        anyhow::bail!("WinFSP support not enabled. Enable the 'winfsp-vfs' feature.")
    }
}

#[cfg(not(feature = "winfsp-vfs"))]
pub use stub::{mount_winfsp, unmount_winfsp, WindowsVfsBackend};

// ─── Full implementation (winfsp-vfs feature enabled) ────────────────────

#[cfg(feature = "winfsp-vfs")]
mod imp {
    use std::{
        collections::HashMap,
        ffi::OsStr,
    ...
    impl FileSystemContext for WinFspVfs {
        ...
        fn rename(
            ...
            if let Ok(Some(target_meta)) = self.block_on(db::lookup_by_parent_and_name(
                ...
                if let Err(e) =
                    self.block_on(db::soft_delete_by_inode(&self.ctx.db, target_meta.inode))
                {
    ...
    pub async fn mount_winfsp(ctx: VfsContext) -> anyhow::Result<VfsHandle> {
        ...
        let host =
            tokio::task::spawn_blocking(move || FileSystemHost::new(fs, &mount_str, &params))
                .await
...
#[cfg(feature = "winfsp-vfs")]
pub use imp::{mount_winfsp, unmount_winfsp, WindowsVfsBackend};
